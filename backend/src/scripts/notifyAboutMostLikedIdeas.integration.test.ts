...
import {
  getMostLikedIdeas,
  notifyAboutMostLikedIdeas,
} from './notifyAboutMostLikedIdeas';

const createData = async (now: Date) => {
  // has 3 likes in prev month
  const { idea: idea1, author: author1 } = await createIdeaWithAuthor({
    number: 1,
  });

  // has 2 like in prev month, and 2 like in prev prev month
  const { idea: idea2, author: author2 } = await createIdeaWithAuthor({
    number: 2,
  });

  // has 1 like in prev month, and 1 like in prev prev month
  const { idea: idea3, author: author3 } = await createIdeaWithAuthor({
    number: 3,
  });

  // has 3 likes in prev prev month
  const { idea: idea4, author: author4 } = await createIdeaWithAuthor({
    number: 4,
  });
...
describe('getMostLikedIdeas', () => {
  it('return most liked ideas of prev month', async () => {
    ...
    expect(
      withoutNoize(await getMostLikedIdeas({ ctx: appContext, limit: 2, now }))
    ...
    `);
    expect(
      withoutNoize(await getMostLikedIdeas({ ctx: appContext, limit: 10, now }))
    ).toMatchInlineSnapshot(`
[
  {
    "name": "Idea 1",
